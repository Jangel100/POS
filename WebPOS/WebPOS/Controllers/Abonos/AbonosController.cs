...
namespace WebPOS.Controllers.Abonos
{
    ...
    public class AbonosController : Controller
    {
        ...
        // GET: Abonos
        public AbonosController(IAbonosBL abonosBL)
        {
            _AbonosBL = abonosBL;
        }
        ...
        {
            ClentesAbonos ClentesAbonos;
            ViewBag.Idstore = Session["IDSTORE"].ToString();
            ViewBag.WhsID = Session["WHSID"].ToString();
            ClentesAbonos = Cargaclientes();
            return View("Abonos", ClentesAbonos);
        }

        public JsonResult Buscapedido(string folio)
        {
        ...
        public List<AddPay> DoBind(AddPay abono)
        {
            ...
            try
            {
                urlp = "api/searchpedido";
                HttpResponseMessage response = _AbonosBL.ReadAsStringAsyncAPI(urlp, abono);
        ...
        public JsonResult AddAbono(string Abono)
        {
            ...
            String archivo_logpago = AppDomain.CurrentDomain.BaseDirectory + @"logErrorPayback\Logabono_" + System.DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss").Replace("T", "_").Replace("-", "_").Substring(0, 10) + ".txt";
            ...
            try
            {
                ...
                {
                    url = "api/AddPay";
                    HttpResponseMessage response = _AbonosBL.ReadAsStringAsyncAPI(url, jsonpago);
                    response.EnsureSuccessStatusCode();
        ...
        public JsonResult SearchCli(string name)
        {////con typeahed o autocomplete
            string JsonResult = string.Empty;
            string squery = "";
            ...
            try
            {
                squery = "select distinct CL.Nombre + ' ' +CL.RFC+ ' Nc:' + CONVERT(varchar(10), CL.ID) as Nombre from ventasencabezado VE, Clientes CL where VE.IDCliente = CL.IDand idStore = " + idstore + "and CL.Nombre LIKE '%" + name + "%'";
                dt = oDB.EjecutaQry_Tabla(squery, CommandType.Text, "Clientes", connstringWEB);
                if (dt != null)
                {
                    var Clientes = (from DataRow rows in dt.Rows
                                    select new ClentesAbonos
                                    {
        ...
        public ClentesAbonos Cargaclientes()
        {
            try
            {
                ClentesAbonos ClentesAb;
                connstringWEB = ConfigurationManager.ConnectionStrings["DBConn"].ConnectionString;
                ...
                var ListClientes = GetClineteabono(connstringWEB);
                ClentesAb = new ClentesAbonos()
                {
        ...
        public List<ListClientes> GetClineteabono(string connstringWEB)
        {
        ...
        [HttpPost]
        public JsonResult GetPeriods(string idcliente)
        {
            string JsonResult = string.Empty;
            try
            {
                var result = _AbonosBL.GetPeriods(idcliente);
        ...
        [HttpPost]
        public JsonResult GetDays(string idcliente, string date)
        {
            ...
            try
            {
                var result = _AbonosBL.GetDays(jsonparams);
                if (result != null) { JsonResult = JsonConvert.SerializeObject(result); }
                return Json(JsonResult);
            }
            catch (Exception ex)
        ...
        [HttpPost]
        public JsonResult GetFolios(string idcliente, string date, string day)
        {
            string jsonparams = string.Empty;
            var idstore = Session["IDSTORE"].ToString();
            var Jsonquery = new
            ...
            try
            {
                var result = _AbonosBL.GetFolios(jsonparams);
                if (result != null) { JsonResult = JsonConvert.SerializeObject(result); }
                return Json(JsonResult);
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (result != null)
                    {
                        ...
                        HttpResponseMessage responseAc = _AbonosBL.ReadAsStringAsyncAPI(url, ListaItemsAcumulation);
                ...
                if (!string.IsNullOrEmpty(jsonpago.Monedero))// si cumple con las condiciones entra y acumula puntos
                {
                    ...
                    {
                        objR = _AbonosBL.CreateObjRegisterPaybackSale(jsonpago.Id, request1.branchShortName, request1.Fecha, 0, _TotalV, 0, jsonpago.Monedero, jsonpago.SecretPassword, AdminAc.PartnerShortName, "", "", "", "AcumulacionVenta", resultAc, false, "", 0, 0, 0, Idabono);
                        payback.RegisterPaybackSales(objR);
                        acomulacion = false;
                    }
