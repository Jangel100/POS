VentasFranquicas = {
    modalBuscacliente: function () {
        $("#idbuscaclienteF").click(function () {
            VentaUsuarioFranquicias.isRequiredFacturar();
        });
    }
};
var arayJson = '';
...
VentaUsuarioFranquicias = {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        $('#tblConsultaClientesEnc tbody').on('click', 'tr', function () {
                            ...
                            VentaUsuarioFranquicias.showClientes(data[0], data[2]);
                        });
    ...
    showClientes: function (idcliente, RFC) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $("#txtCPFact").val('');
                $("#txtEstadosFact").val('');
                //VentaUsuarioFranquicias.enabledTxtApellidos();
                //******Se limpia los valores de Docimilio al realizar una nueva busqueda******//

                if (ls.ClientesFacturacion != null) {
                    VentaUsuarioFranquicias.loadDatosPedidos(ls.Clientes);
                    VentaUsuarioFranquicias.GetCPFacturacion(ls.Clientes.actCod_COMUNAColonia, ls.Clientes.CP, ls.ClientesFacturacion.CP, "");
                    VentaUsuarioFranquicias.SearchCPLoadN(ls.Clientes.actCod_COMUNAColonia, "");

                    VentaUsuarioFranquicias.loadDatosClienteFacturacion(ls.ClientesFacturacion);
                    VentaUsuarioFranquicias.GetCPFacturacionFact(ls.ClientesFacturacion.actCod_COMUNAcolonia, ls.ClientesFacturacion.CP, ls.Clientes.CP, "Fact");
                    //VentaUsuarioFranquicias.SearchCPLoadN(ls.ClientesFacturacion.actCod_COMUNAcolonia, "Fact");
                    ...
                    VentaUsuarioFranquicias.loadDatosPedidos(ls.Clientes);
                    VentaUsuarioFranquicias.GetCP(ls.Clientes.actCod_COMUNAColonia, "");
                }
    ...
    SelectedDatosIguales: function () {
        $("#chkCheckDatosIguales").change(function () {
            if ($(this).prop('checked')) {
                ...
                $("#txtCPFact").val(txtCP);
                VentaUsuarioFranquicias.SearchCPLoad(txtColonia, 'Fact');
    ...
    CapturarCliente: function () {
        $("#btnCapturarClienteF").click(function () {

            let validar = true;

            validar = VentaUsuarioFranquicias.Validateform("Registrar");
                    ...
                    success: function (result) {
                        if (result == '-1') {
                            ...
                            if (result == '3') {
                                ...
                                VentaUsuarioFranquicias.dataSeleccionarCliente(result);
                            }
                        }
    ...
    CapturarClienteN: function () {
        $("#btnCapturarAdd").click(function () {
            ...
            //if (VentaUsuarioFranquicias.isValidDate(fecha) == false) {
    ...
    ActualizarClienteN: function () {
        $("#btnActualizarF").click(function () {
            ...
            var validate = VentaUsuarioFranquicias.Validateform("Actualizar");
            if (validate == true) {
    ...
    SeleccionarCliente: function () {
        $("#btnSeleccionarF").click(function () {
            let txtIdCliente = $('input[name=txtIdCliente]').val();

            let IsValidate = false;

            IsValidate = VentaUsuarioFranquicias.Validateform("ActualizarFact");

            if (IsValidate == true) {
                VentaUsuarioFranquicias.ActualizarFactClienteN(); //Actualiza Direccion de Facturacion en caso de que no tenga
                VentaUsuarioFranquicias.dataSeleccionarCliente(txtIdCliente);
            }
        });
    },
    ClearBtnclienteAdd: function () {
        $('#btnClearclienteAdd').click(function () {
            VentaUsuarioFranquicias.CleanAddCliente();
        });
    },
    CloseModalCliente: function () {
        $("#BuscaclienteModal").on("hidden.bs.modal", function () {
            VentaUsuarioFranquicias.ClearCliente();
        });
    },
    ...
    CloseModalClienteNew: function () {
        $("#BuscaclienteModal").on("hidden.bs.modal", function () {
            VentaUsuarioFranquicias.clearClient();
            $('#cadClienteEncontrados').hide();
        });
    },
    ...
    ClearDatosCliente: function () {
        $('#btnClearDatosClientesF').click(function () {
            VentaUsuarioFranquicias.ClearCliente();
        });
    },
    ...
    ValidarCP: function (Tipo) {
        VentaUsuarioFranquicias.SearchCPLoad("", Tipo);
    },
    ...
    GetCP: function (Colonia, Tipo) {
        VentaUsuarioFranquicias.SearchCPLoad(Colonia, Tipo);
    },
            ...
            success: function (data) {
                ...
                if (dataset != null) {
                    ...
                    $("#tblCPEncontrados" + Tipo + " tbody").on('click', 'tr', function () {
                        ...
                        VentaUsuarioFranquicias.SearchCPNew(CP, colonia, Tipo);
    ...
    onchangeCheckAddF: () => {
        $('#btnActualizarF').hide();
        $('#btnSeleccionarF').hide();
        $("#inlineCheckAgregarF").change(() => {
            if ($("#inlineCheckAgregarF").prop('checked')) {
                $('#txtRFC').focus();
                $("#cmboTipoPersonaF").removeAttr("disabled");
                $("#txtRFC").attr("disabled", true);
                $("#txtNombre").attr("disabled", true);
                $("#btnBuscarClienteF").attr("disabled", true);
                VentaUsuarioFranquicias.clearClient();
                $('#btnActualizarF').hide();
                $('#btnSeleccionarF').hide();
                $("#btnCapturarClienteF").show();
                document.getElementById("txtName").focus();
                $(".cssCheckAdd").find(".switch-off").addClass("btn btn-info");
                $('.cardBusquedaClienteF').hide();
            }
            else {
                $("#txtRFC").removeAttr("disabled");
                $('#txtName').focus();
                $("#txtNombre").removeAttr("disabled");
                $("#btnBuscarClienteF").removeAttr("disabled");
                $("#txtRFCN").val('');
                $("#checkrfc").prop("checked", false);
                $('#btnActualizarF').show();
                $('#btnSeleccionarF').show();
                $("#btnCapturarClienteF").hide();
                $('.cardBusquedaClienteF').show();
    ...
    actionCheck: () => {
        $("#cmboTipoPersonaF").attr("disabled", true);
        $('input[name=optionRequiredFact]').change(() => {
            if ($('input[name=optionRequiredFact]:checked').val() == 2) {
                ...
                $("#txtRFCN").attr('maxlength', 13);
                //VentaUsuarioFranquicias.disableTxtApellidos();
    ...
    showModalCliente: () => {
        $('#BuscaclienteModal').on('show.bs.modal', function (e) {
            ...
            VentaUsuarioFranquicias.modifyCheckRFC();
            VentaUsuarioFranquicias.modifyCheckAdd();
            VentaUsuarioFranquicias.removeTextcheck();
            VentaUsuarioFranquicias.SolonumerosyLetrasNombre();
            //VentaUsuarioFranquicias.disableTxtApellidos();
    ...
    onchangeTipoPersona: () => {
        $("#cmboTipoPersonaF").change(() => {
            if ($("#cmboTipoPersonaF").val() == "Fisica") {
    ...
    disabledTipoPersona: () => {
        $("#cmboTipoPersonaF").val("Fisica");
        if ($("#cmboTipoPersonaF").val() == "Fisica") {
            $("#cmboTipoPersonaF").attr('disabled', true);
        }
        else {
            $("#cmboTipoPersonaF").removeAttr('disabled', true);
        }
    },
    loadDataListFacturacion: () => {
        $('#ConfirmarventaModalF').on('show.bs.modal', () => {
    ...
    showTabVentasSiguienteClientes: () => {
        $('#btnTabSiguienteClientesF').click(() => {
            var pasa = VentaUsuarioFranquicias.requiredTipoFactura();
            if (pasa == 'true') {
                $('.nav-tabs a[href="#articulos-tab"]').tab('show');
                document.getElementById("datalistmodelosF").focus();
            }
            let name = $('#nameV').val();
            if ($('input[name=optionRequiredFact]:checked').val() == 2) {
    ...
    showTabArticulos: () => {
        $('#btnTabSiguientePagosF').click(() => {
            $('.nav-tabs a[href="#pagos-tab"]').tab('show')
        });
        $('#btnTabAtrasClientesF').click(() => {
    ...
    showTabDireccionPedido: () => {
        $('#btnContinuarDatosPedidosF').click(() => {
    ...
    showTabDireccionFacturacion: () => {
        $('#btnContinuarDatosFacturacionF').click(() => {
            $('.nav-tabs a[href="#nav-dir-facturacion"]').tab('show');
        });
        $('#btnAtrasDatosGeneralF').click(() => {
            $('.nav-tabs a[href="#nav-home-general"]').tab('show');
        });
    },
    isRequiredFacturar: () => {
        let isRequired = VentaUsuarioFranquicias.validaRequiredFactura();
        let tipodeventaF = $("#tipodeventaF").val();
        if (typeof isRequired == 'undefined') {
            alert("Seleccione la opcion por favor 'El Cliente requiere factura'");
            return false;
        }
        else if (tipodeventaF === 'seleccionet') {
            document.getElementById("tipodeventaF").focus();
    ...
    disabledTab: () => {
        VentaUsuarioFranquicias.enabledTab();
    ...
    validOnchangeTipodeventa: () => {
        $('#tipodeventaF').change(() => {
            let tipodeventaF = $("#tipodeventaF").val();
            if (tipodeventaF === 'seleccionet') {
                VentaUsuarioFranquicias.disabledTab();
            }
            else {
                VentaUsuarioFranquicias.enabledTab();
            }
    ...
    MostrarModalColonia: function (Tipo) {
        VentaUsuarioFranquicias.CaseModal(Tipo);
    },
...
$(document).ready(function () {
    ...
    $('#divRFCF').append("<input type='text' class='form-control focus' id='txtRFC' maxlength='13' autofocus disabled>");
    //document.getElementById("txtRFC").focus();
    VentaUsuarioFranquicias.disabledCheckRfc();
    VentaUsuarioFranquicias.GetUsuario();
    VentaUsuarioFranquicias.DeleteClientesEncontrados();
    VentaUsuarioFranquicias.SelectedDatosIguales();
    VentaUsuarioFranquicias.CapturarCliente();
    VentaUsuarioFranquicias.SeleccionarCliente();
    VentaUsuarioFranquicias.ActualizarClienteN();
    VentaUsuarioFranquicias.CloseModalCliente();
    VentaUsuarioFranquicias.CloseModalClienteNew();
    VentaUsuarioFranquicias.CapturarClienteN();
    VentaUsuarioFranquicias.ClearDatosCliente();
    VentaUsuarioFranquicias.ClonarVenta();
    VentaUsuarioFranquicias.SearchVentaC();
    VentaUsuarioFranquicias.ClearBtnclienteAdd();
    VentaUsuarioFranquicias.SolonumerosCP();
    VentaUsuarioFranquicias.SoloNumeroLetrasRFC();
    VentaUsuarioFranquicias.onchangeCheckAddF();
    VentaUsuarioFranquicias.actionCheck();
    VentaUsuarioFranquicias.showModalCliente();
    VentaUsuarioFranquicias.onchangeTipoPersona();
    VentaUsuarioFranquicias.loadDataListFacturacion();
    //VentaUsuarioFranquicias.disabledSelectFacturacion();
    VentaUsuarioFranquicias.showTabVentasSiguienteClientes();
    VentaUsuarioFranquicias.showTabActive();
    VentaUsuarioFranquicias.showTabArticulos();
    VentaUsuarioFranquicias.showTabPagos();
    VentaUsuarioFranquicias.showTabDireccionPedido();
    VentaUsuarioFranquicias.showTabDireccionFacturacion();
    VentaUsuarioFranquicias.valTipoTarjeta();
    VentaUsuarioFranquicias.validOnchangeTipodeventa();
    VentasFranquicas.modalBuscacliente();
    VentaUsuarioFranquicias.loginGarantias();
    VentaUsuarioFranquicias.loginGarantiasOficinas();
});
...
function CapPagoF() {
    ...
    let nameFormaPago = '';
    nameFormaPago = VentaUsuarioFranquicias.getFormaPago(formapago);
...
///////////Pago termina
$("#ctbiendaF").change(function () {
    var idCheckJgo = document.getElementById('juego');
    var ValuecTextboxTienda = $("#ctbiendaF").val();
    var ValueTextboxBodega = $("#ctbodegaF").val();
    var ValueExistenciaTienda = $("#ctienda1F").val();
    var ddlLinea = $("#ddlPriceF option:selected").val();
    var ddlArticulo = $("#modelocode").val();
    var Texboxdescuento = $("#tbDescuento").val();
    if (ValuecTextboxTienda == "") {
        CleanPriceF(false, false);
    ...
    if (parseInt(ValueTextboxBodega) < 1) {
        ...
        CleanPriceF(true, false);
    }


});
$("#ctbodegaF").change(function () {
    var idCheckJgo = document.getElementById('juego');
    var ValueTextboxBodega = $("#ctbodegaF").val();
    var ValuecTextboxTienda = $("#ctbiendaF").val();
    var ValueExistenciaBodega1 = $("#cbodega1F").val();
    var ddlLinea = $("#ddlPriceF option:selected").val();
    var ddlArticulo = $("#modelocode").val();
    var Texboxdescuento = $("#tbDescuento").val();
...
$("#calmacen").change(function () {
    var idCheckJgo = document.getElementById('juego');
    var ValueTextboxBodega = $("#ctbodegaF").val();
    var ValuecTextboxTienda = $("#ctbiendaF").val();
    var ValuecTextboxAlmacen = $("#calmacen").val();
    var ValueExistenciaBodega1 = $("#cbodega1F").val();
    var ddlLinea = $("#ddlPriceF option:selected").val();
    var ddlArticulo = $("#modelocode").val();
    var Texboxdescuento = $("#tbDescuento").val();
   
    if (ValuecTextboxAlmacen == "") {
        CleanPriceF(false, false);
        MessageError("El monto no puede quedar vacio");
        return;
    } else if (ValuecTextboxAlmacen != "" && ValuecTextboxTienda == "") { ValuecTextboxTienda = 0 }
    if (parseInt(ValuecTextboxTienda) < 1) {
        if (ValidaCeros(ValuecTextboxAlmacen)) {
            if (ValuecTextboxAlmacen == 0) {
                CleanPriceF(true, false);
                MessageError("El valor debe ser mayor a 0");
                return false;
            } else {
                var obj = { ItemCode: ddlArticulo, IdList: ddlLinea, descuento: Texboxdescuento, cantidad: ValuecTextboxAlmacen, Origen: "Bodega", Juego: idCheckJgo.checked };
                var Json = JSON.stringify(obj);
                ChangePriceF(Json);
            }
        }
...
function ChangePriceF(JsonChangePrice) {
    $.ajax({
        type: "POST",
        url: '/VentasFranquicias/GetSelectChanchePrice',
...
function CleanPriceF(CleanStock, CleanModel) {
    event.preventDefault();
    if (CleanStock == true && CleanModel == true) { OnchangeddlModeloF("Linea"); }
    if (CleanStock) {
        $("#ctienda1F").val(0);
        $("#cbodega1F").val(0);
    }
    ...
    $("#ctbiendaF").val("");
    $("#ctbodegaF").val("");
    $("#datalistOptionsF").val("");
    $("#cbDescriptioJuego").val("");
    $("#tbDescuento").val("");
    $("#modelocode").val("");
    $('#datalistmodelosF').val("");
    ...
    PriceLoadF(); //recarga lista de precios al valor Default
    var idCheckJgo = document.getElementById('juego');
    var idCheckLineF = document.getElementById('idCheckLineF');
    idCheckJgo.checked = false;
    idCheckLineF.checked = true;
}
function CheckLineF() {

    var idCheckDescF = document.getElementById('idCheckDescF');
    var idCheckLineF = document.getElementById('idCheckLineF');
    if (idCheckLineF.checked) {
        OnchangeddlModeloF("Linea");
        CleanPriceF(true, false);
        idCheckDescF.checked = false;
        idCheckLineF.checked = true;
...
function CheckDescF() {
    var idCheckLineF = document.getElementById('idCheckLineF');
    var idCheckDescF = document.getElementById('idCheckDescF');
    if (idCheckDescF.checked) {
        OnchangeddlModeloF("Descontinuados");
        CleanPriceF(true, false);
        idCheckLineF.checked = false;
        idCheckDescF.checked = true;
        $("#articuloen").val("Descontinuados");
    } else {
        idCheckDescF.checked = true;
    }
}
function OnchangeddlModeloF(tipo) {
    $.ajax({
        type: "POST",
        url: '/VentasFranquicias/GetSelectOnchangeModelo',
        data: "{ 'tipo': '" + tipo + "'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (result) {
...
function CapturarF() {
    ...
    var ddlLinea = $("#ddlPriceF option:selected").text();
    var modelo = $('#datalistmodelosF').val();
    var ExistTienda = false;
    var ExistBodega = false;
    var ExistAlmacen = false;
    ...
    if (ddlLinea == undefined || ddlLinea == ":.Seleccione.:") {
        ...
        var descripcionitem = $('#datalistmodelosF').val();
        var idCheckJgo = document.getElementById('juego');
        var ValuecTextboxTienda = $("#ctbiendaF").val();
        var ValueTextboxBodega = $("#ctbodegaF").val();
        var ValueTextboxAlmacen = $("#calmacen").val();
        var ddlArticulo = $("#modelocode").val();
        var ddlModelo = $('#datalistmodelosF').val();

        var Texboxdescuento = $("#tbDescuento").val();
        var origen = GetOrigenF(ValuecTextboxTienda, ValueTextboxBodega, ValueTextboxAlmacen);
        var idCheckJgo = document.getElementById('juego');
        if (origen == "Favor de insertar un monto válido") {
            ...
            switch (origen) {
                case "Tienda":
                    cantidadorigen = ValuecTextboxTienda;
                    break;
                case "Bodega":
                    cantidadorigen = ValueTextboxBodega;
                    break;
                case "Almacen":
                    cantidadorigen = ValueTextboxAlmacen;
                    break;
            }

            var obj = { ItemCode: ddlArticulo, IdList: ddlLinea, descuento: Texboxdescuento, cantidad: cantidadorigen, Origen: origen, Juego: idCheckJgo.checked };
            var Json = JSON.stringify(obj);
        }
        if (origen == "Tienda") {
            var ValueExistenciaTienda = $("#ctienda1F").val();
        ...
        $.ajax({
            type: "POST",
            url: '/VentasFranquicias/GetSelectedJgo',
            ...
            success: function (result) {
                ...
                if (result != null) {
                    ...
                    lsLinea.forEach(function (element) {
                        ...
                        else if (element.CAlmacen > 0) { ExistAlmacen = true; }
                        if (!idCheckJgo.checked) { element.Lista = $("#ddlPriceF option:selected").text(); }

                        table.append('<tr width="1000"  id="row' + i + '"><td width="1000">' + element.Modelo + "</td><td hidden='" + "hidden" + "'>" + i + "</td><td>" + element.Juego + "</td><td>" + element.Cantidad + "</td><td>" + element.CTienda + "</td><td>" + element.CBodega + "</td><td " +
                            (Customer != "C2021" ?  "hidden='hidden'" : "")   + " >" + element.CAlmacen + "</td><td>" + element.Lista + "</td><td>" + element.PrecioUnitario + "</td><td>" + element.Descuento + "</td><td>" + element.Subtotal + "</td><td>" + element.IVA + "</td><td>" + element.Total + "</td><td hidden='hidden'>" + element.ItemCode + '</td><td><button type="button" name="remove" id="' + i + '" class="btn btn-danger btn-sm btn_remove1"><i class="fa fa-trash" aria-hidden="true"></i>Eliminar</button></td></tr>');
...
function GetCantidad() {
    ...
    if ($("#ctbiendaF").val() > 0) {
        cantidad = $("#ctbiendaF").val();
    } else { cantidad = $("#ctbodegaF").val(); }
    return cantidad;
...
function LoadpreviewerF() {
    ...
    if (cantPagos > 1) {        //Si seleciona varias formas de Pagos
        ...
        valFormaPago = VentaUsuarioFranquicias.getNumberFormaPago(TipoPago);
    ...
    if (typeof valTipoVenta == 'undefined') {
        ...
        VentaUsuarioFranquicias.loadDataListFormaPago(valTipoVenta, valFormaPago);
    }
    ...
    $("#ConfirmarventaModalF").modal({ backdrop: 'static', keyboard: false });
...
function LoadCapturedItems() {
    ...
    $('#tbBodyIn tr').each(function () {
        ...
        var total = $(this).find("td").eq(12).html();
        if (total == "" || total == undefined) {
...
$("#paymetsidF").on('submit', (e) => {
        ...
        if (isTrueTipoDeVenta == 'true') {
            ...
            $('#tbBody tr').each(function (row, tr) {
                TableArticulos[row] = {
                    ...
                    , "CantidadAlmacen": $(tr).find('td:eq(6)').text()
                    , "Lista": $(tr).find('td:eq(7)').text()
                    , "PrecioUnitario": $(tr).find('td:eq(8)').text()
                    , "Descuento": $(tr).find('td:eq(9)').text()
                    , "subTotal": $(tr).find('td:eq(10)').text()
                    , "IVA": $(tr).find('td:eq(11)').text()
                    , "Total": $(tr).find('td:eq(12)').text()
                    , "Linea": $(tr).find('td:eq(13)').text()
            ...
            if (IsTrue == true) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        var Data;
                        var IdVenta;
                        var typeofsale = document.querySelector('#tipodeventaF option:checked');
                        ...
                        var Entregainmediata = document.getElementById("entregainmediata"); 
                        let Entregain = (Entregainmediata.checked == true) ? 1 : 0;
                        VentasFranquicas.modalBuscacliente();
                        ...
                        $.ajax({
                            ...
                            success: function (msg) {
                                ...
                                if (result.MessageErrorPayback == "" || result.MessageErrorPayback == null) {
                                    ...
                                    window.location.href = "/VentasRPTS/Download_VentaDormimundoFranquicias_PDF?IDVENTA=" + IdVenta;
    ...
    function Confirmacion(IdVenta) {
        ...
        Swal.fire({
            ...
            $("#ConfirmarventaModalF").modal('hide');
            Swal.fire(
                '¡Operacion realizada!',
            ...
            if (result.isConfirmed) {
                ...
                let rutaReports = 'https://' + Url[2] + '/VentasFranquicias';
...
$(document).ready(function () {
    ...
    //DATALIST MODELOS
    $("#datalistmodelosF").on('input', function () {
        $("#cbIva").val("");
        $("#tbpunitario").val("");
        $("#tbTotal").val("");
        $("#tbDescuento").val("");
        var val = $('#datalistmodelosF').val();
        var articuloen = $('#articuloen').val();
        if (articuloen == "") { articuloen = 'Linea' }
        var ddlModelo = $('#datalistOptionsF').find('option[value="' + val + '"]').data('code');
        if (ddlModelo == undefined || ddlModelo == '') {
            //nothing
        } else {
            if (ddlModelo == '1010770061' || ddlModelo == '1010770062' || ddlModelo == '1010770011' || ddlModelo == '1010770012' || ddlModelo == '1010770014') {
                $('#entregainmediata').prop('checked', true);
                $('#entregainmediata').attr('disabled', true);
            }
            $("#modelocode").val(ddlModelo);
            $.ajax({
                type: "POST",
                url: '/VentasFranquicias/GetExistencias',
                ...
                success: function (result) {
                    if (result != null) {
                        ...
                        $("#ctienda1F").val(date.ExistenciaTienda);
                        $("#cbodega1F").val(date.ExistenciaBodega);
                        $("#calmacen1").val(date.ExistenciaAlmacen);
                    }
                    else { alert("Existen datos"); }
                    return false;
                },
    ...
    //DATALIST CLAVE CORTA
    $("#datalistmodeloscve").on('input', function () {
        $("#cbIva").val("");
        $("#tbpunitario").val("");
        $("#tbTotal").val("");
        $("#tbDescuento").val("");
        var val = $('#datalistmodeloscve').val();
        var articuloen = $('#articuloen').val();
        if (articuloen == "") { articuloen = 'Linea' }
        var ddlModelo = $('#datalistOptionscve').find('option[value="' + val + '"]').data('code');
        if (ddlModelo == undefined || ddlModelo == '') {
            //nothing
        } else {
            $("#modelocode").val(ddlModelo);
    ...
    //DATALIST CLAVE CODIGO
    $("#datalistmodeloscode").on('input', function () {
        $("#cbIva").val("");
        $("#tbpunitario").val("");
        $("#tbTotal").val("");
        $("#tbDescuento").val("");
        var val = $('#datalistmodeloscode').val();
        var articuloen = $('#articuloen').val();
        if (articuloen == "") { articuloen = 'Linea' }
        var ddlModelo = $('#datalistOptionscode').find('option[value="' + val + '"]').data('code');
        if (ddlModelo == undefined || ddlModelo == '') {
            //nothing
        } else {
            $("#modelocode").val(ddlModelo);
...
function CheckJuegoF() {
    var idCheckJgo = document.getElementById('juego');
    var ddlModelo = $("#modelocode").val();
    var TypeArt = $("#tipodeventaF option:checked").text();//obtiene tipo de venta
...
function GetOrigenF(montoT, montoB, montoA) {
    if ((montoT == 0 && montoB == 0 && montoA == 0) || (montoT != 0 && montoB != 0 && montoA != 0) ) {
...
function PriceLoadF() {
    var TypeArt = $("#tipodeventaF option:checked").text();
...
function filterswatchF() {
    var checkfiltro = document.getElementById("inlineCheckFilter");
    var x = document.getElementById("filtrosartF");
...
function OnchangeListPriceLoadF() {
    PriceLoadF();
}
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                data: "{ 'ItemCode': '" + ItemCode + "','monto': '" + monto + "','idcliente': '" + idcliente + "','Observaciones': '" + Observaciones + "'}",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (result) {
                    if (result != null) {
